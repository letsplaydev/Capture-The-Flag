...
class CTFManager {
	 ...
	 * @param CTF $pg        	
	 */
	public function __construct(CTFPlugIn $pg) {
		$this->plugin = $pg;
	}
	...
	public function stopGame(Player $player) {
		...
		$this->getBlockBuilder ()->closeTeamGates ( $player->level, $arenaSize, $arenaPos->x + $arenaSize, $arenaPos->y, $arenaPos->z );		
		return;
	}
	...
	public function handleNewGame(Player $player) {
		...
		$this->getBlockBuilder ()->closeTeamGates ( $level, $arenaSize, $arenaPos->x + $arenaSize, $arenaPos->y, $arenaPos->z );
		// reset fire
		// blue team flag
		$this->getBlockBuilder ()->addBlueTeamFlag ( $level, 171, 11 );
		// add red team flag
		$this->getBlockBuilder ()->addRedTeamFlag ( $level, 171, 14 );
		// add game button
		$this->getBlockBuilder ()->addGameButtons ( $level );
		// reset stats
		$this->getPlugIn ()->blueTeamWins = 0;
		$this->getPlugIn ()->redTeamWins = 0;
	}
	...
	public function checkRedTeamCapturedEnermyFlag(Player $player, Level $level, $rb) {
		...
		if (round ( $rb->x ) == round ( $redTeamEnemyFlagPos->x ) && round ( $rb->y ) == round ( $redTeamEnemyFlagPos->y ) && round ( $rb->z ) == round ( $redTeamEnemyFlagPos->z )) {
			...
			if ($blueflag->getId () != 0) {
				...
				// erase placed block
				$this->getBlockBuilder ()->resetBlock ( $rb, $level, 0 );
				return;
			}
	...
	public function handleNextRoundOfGame(Level $level) {
		...
		if ($this->getPlugIn ()->currentGameRound < $this->getPlugIn ()->maxGameRound) {
			...
			$this->getPlugIn ()->getServer ()->getScheduler ()->scheduleDelayedTask ( $resetTask, $resetValue );
	...
	public function buildGameArena(CommandSender $sender) {
		...
		$this->getBlockBuilder ()->plotArenaMap ( $level, $arenaSize, 8, $arenaPos->x, $arenaPos->y, $arenaPos->z, 1 );
		$this->getPlugIn ()->gameMode = 0;
	...
	public function handleStartTheGame(Level $level) {
		...
		$this->getBlockBuilder ()->addRoundPlayButtons ( $level );
		...
		$arenaSize = $this->getSetup ()->getArenaSize ();
		$arenaPos = $this->getSetup ()->getGamePos ( CTFSetup::CTF_GAME_ARENA_POSITION );
		$this->getBlockBuilder ()->openTeamGates ( $level, $arenaSize, $arenaPos->x + $arenaSize, $arenaPos->y, $arenaPos->z );
	...
	public function handlePlayerQuit(Player $player) {
		...
		if (isset ( $this->getPlugIn ()->blueTeamPlayers [$player->getName ()] )) {
			...
			if ($player->getInventory ()->contains ( new Item ( 171 ) )) {
				...
				$this->getBlockBuilder ()->addRedTeamFlag ( $player->getLevel (), 171, 14 );
				// remove it from player
				$player->getInventory ()->remove ( new Item ( 171 ) );
			}
		}
	...
	public function getGameKit() {
		return $this->getPlugIn ()->ctfGameKit;
	}

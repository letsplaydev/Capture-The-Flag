...
class CTFGameKit {
	...
	private function addBluePlayerAmour(Player $p) {
		//$p->sendMessage ( $this->getMsg ( "game.start-equipment" ) );
		$blueteamKit = $this->getKit(self::KIT_BLUE_TEAM_NAME)->getAll();
	...
	private function addRedTeamPlayerAmor(Player $p) {
		//$p->sendMessage ( $this->getMsg ( "game.start-equipment" ) );
		
		// set player chest
		$p->getInventory ()->setHelmet ( new Item ( 302, 0, 1 ) );
		$p->getInventory ()->setChestplate ( new Item ( 303, 0, 1 ) );
		$p->getInventory ()->setLeggings ( new Item ( 304, 0, 1 ) );
		$p->getInventory ()->setBoots ( new Item ( 305, 0, 1 ) );
		// notify viewers
		$p->getInventory ()->sendArmorContents ( $p );
		// fully health
		$p->setHealth ( 20 );
		// add sword, bow and arrow
		if (! $p->getInventory ()->contains ( new Item ( 272 ) )) {
			$p->getInventory ()->addItem ( new Item ( 272, 0, 1 ) );
		}
		if (! $p->getInventory ()->contains ( new Item ( 50 ) )) {
